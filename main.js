...
function _VirtualDom_render(vNode, eventNode, tNode)
{
	...
	if (tag === 4)
	{
		return _VirtualDom_render(vNode.k, function (msg, isSync) { return eventNode(vNode.j(msg), isSync) }, tNode);
	}
...
function _VirtualDom_renderTranslated(vNode, eventNode, tNode)
{
	...
	if (tag === 4)
	{
		return _VirtualDom_renderTranslated(vNode.k, function (msg, isSync) { return eventNode(vNode.j(msg), isSync) }, tNode);
	}
...
function _VirtualDom_diffHelp(x, y, eventNode, tNode)
{
	...
	if (y.$ === 4)
	{
		return _VirtualDom_diffHelp(x, y.k, function (msg, isSync) { return eventNode(y.j(msg), isSync) }, tNode);
	}
...
function _VirtualDom_quickVisit(x, y, eventNode, tNode)
{
	...
	{
		case 4:
			return _VirtualDom_quickVisit(x.k, y.k, function (msg, isSync) { return eventNode(y.j(msg), isSync) }, tNode);
...
var $author$project$Main$emptyWords = A2(
	$elm$core$List$map,
	function (i) {
		return {id: i, word: ''};
	},
...
var $author$project$Main$init = function (_v0) {
	return _Utils_Tuple2(
		{canShowError: false, dragging: $elm$core$Maybe$Nothing, gridPosition: $elm$core$Maybe$Nothing, input: '', over: $elm$core$Maybe$Nothing, viewportWidth: 1024, words: $author$project$Main$emptyWords},
...
var $author$project$Main$GotGridPosition = function (a) {
	return {$: 'GotGridPosition', a: a};
};
var $elm$browser$Browser$Dom$getElement = _Browser_getElement;
var $author$project$Main$getGridPosition = A2(
	$elm$core$Task$attempt,
	$author$project$Main$GotGridPosition,
	$elm$browser$Browser$Dom$getElement('word-grid'));
...
var $author$project$Main$inputToWords = function (input) {
	var wordsInInput = A2(
		...
		F2(
			function (index, word) {
				return {
					id: index,
					word: $elm$core$String$trim(word)
				};
			}),
		A2(
			$elm$core$List$take,
	...
	var withEmptyWords = function (words) {
		return _Utils_ap(
			words,
			A2(
				$elm$core$List$map,
				...
				A2(
					$elm$core$List$range,
					$elm$core$List$length(wordsInInput),
					15)));
	};
...
var $author$project$Main$swapAt = F3(
	function (i, j, words) {
		if (_Utils_eq(i, j)) {
			...
			var arr = $elm$core$Array$fromList(words);
			var b = A2($elm$core$Array$get, j, arr);
			...
			var swapped = function () {
				...
				if ((_v0.a.$ === 'Just') && (_v0.b.$ === 'Just')) {
					...
					return A3(
						...
						j,
						ta,
						A3($elm$core$Array$set, i, tb, arr));
				} else {
					return arr;
				}
			}();
			return $elm$core$Array$toList(swapped);
		}
	});
var $author$project$Main$swapById = F3(
	function (aId, bId, words) {
		var _v0 = _Utils_Tuple2(
			A2($author$project$Main$findIndex, aId, words),
			A2($author$project$Main$findIndex, bId, words));
		if ((_v0.a.$ === 'Just') && (_v0.b.$ === 'Just')) {
			var i = _v0.a.a;
			var j = _v0.b.a;
			return A3($author$project$Main$swapAt, i, j, words);
		} else {
			return words;
		}
	});
...
var $author$project$Main$validateInput = function (input) {
	var wordList = A2($elm$core$String$split, ',', input);
	var hasDuplicates = function (list) {
		if (!list.b) {
			return false;
		} else {
			var x = list.a;
			var xs = list.b;
			return A2($elm$core$List$member, x, xs) || hasDuplicates(xs);
		}
	};
	return ($elm$core$List$length(wordList) <= 16) && (!hasDuplicates(wordList));
};
		...
		switch (msg.$) {
			case 'InputChanged':
				...
				var nextModel = _Utils_update(
					model,
					{
						canShowError: $author$project$Main$validateInput(newText) ? false : true,
						input: newText,
						words: $author$project$Main$inputToWords(newText)
					});
				return _Utils_Tuple2(nextModel, $author$project$Main$getGridPosition);
			case 'DragStart':
				var tileId = msg.a;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							dragging: $elm$core$Maybe$Just(tileId),
							over: $elm$core$Maybe$Just(tileId)
						}),
			...
			case 'DragEnd':
				var _v2 = _Utils_Tuple2(model.dragging, model.over);
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								...
								over: $elm$core$Maybe$Nothing,
								words: A3($author$project$Main$swapById, idA, idB, model.words)
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{dragging: $elm$core$Maybe$Nothing, over: $elm$core$Maybe$Nothing}),
						$elm$core$Platform$Cmd$none);
				}
			case 'GotViewport':
				var result = msg.a;
				if (result.$ === 'Ok') {
					...
					return _Utils_Tuple2(
						...
						$author$project$Main$getGridPosition);
				} else {
					return _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
				}
			case 'WindowResized':
				...
				if (result.$ === 'Ok') {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								gridPosition: $elm$core$Maybe$Just(
									{left: el.element.x, top: el.element.y})
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					return _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
			...
			default:
				var x = msg.a;
				var y = msg.b;
				var _v5 = _Utils_Tuple2(model.dragging, model.gridPosition);
				if ((_v5.a.$ === 'Just') && (_v5.b.$ === 'Just')) {
					var pos = _v5.b.a;
					var spacing = 8;
					var sizes = $author$project$Main$layoutSizes(model);
					var width = sizes.tileWidth + spacing;
					var relY = y - pos.top;
					var relX = x - pos.left;
					var height = sizes.tileHeight + spacing;
					var row = A3(
						$elm$core$Basics$clamp,
						0,
						3,
						$elm$core$Basics$floor(relY / height));
					var column = A3(
						$elm$core$Basics$clamp,
						0,
						3,
						$elm$core$Basics$floor(relX / width));
					var tileIndex = (row * 4) + column;
					var targetId = A2(
						$elm$core$Maybe$map,
						function ($) {
							return $.id;
						...
						$elm$core$List$head(
							A2($elm$core$List$drop, tileIndex, model.words)));
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{over: targetId}),
...
var $elm$html$Html$Attributes$spellcheck = function (bool) {
	return A2(
		_VirtualDom_attribute,
		'spellcheck',
		bool ? 'true' : 'false');
...
var $author$project$Main$viewWordTile = F4(
	function (model, sizes, index, word) {
		var row = (index / 4) | 0;
		var isOver = _Utils_eq(
			model.over,
			$elm$core$Maybe$Just(word.id));
		var isDragging = _Utils_eq(
			model.dragging,
			$elm$core$Maybe$Just(word.id));
		var overAttrs = ((!isDragging) && isOver) ? _List_fromArray(
			[
				$mdgriffith$elm_ui$Element$Border$width(2)
			]) : _List_Nil;
		var fontSize = function () {
			var length = $elm$core$String$length(word.word);
			return $author$project$Main$isMobile(model.viewportWidth) ? ((length <= 5) ? 20 : ((length === 6) ? 18 : ((length === 7) ? 16 : ((length === 8) ? 14 : ((length === 9) ? 12 : 10))))) : ((length <= 8) ? 20 : ((length <= 9) ? 18 : ((length <= 10) ? 16 : 14)));
		}();
		var dragAttrs = isDragging ? _List_fromArray(
			[
		...
		var backgroundColor = function () {
			switch (row) {
				case 0:
					return A4($mdgriffith$elm_ui$Element$rgba255, 249, 223, 109, 1);
				case 1:
					return A4($mdgriffith$elm_ui$Element$rgba255, 160, 195, 90, 1);
				case 2:
					return A4($mdgriffith$elm_ui$Element$rgba255, 176, 196, 239, 1);
				default:
					return A4($mdgriffith$elm_ui$Element$rgba255, 186, 129, 197, 1);
			}
		}();
		var baseAttrs = _List_fromArray(
			[
				$mdgriffith$elm_ui$Element$width(
				$mdgriffith$elm_ui$Element$px(sizes.tileWidth)),
				$mdgriffith$elm_ui$Element$height(
				$mdgriffith$elm_ui$Element$px(sizes.tileHeight)),
				...
				$author$project$Main$DragStart(word.id)),
				$mdgriffith$elm_ui$Element$Events$onMouseEnter(
				$author$project$Main$DragEnter(word.id)),
				$mdgriffith$elm_ui$Element$Events$onMouseUp($author$project$Main$DragEnd),
				$mdgriffith$elm_ui$Element$htmlAttribute(
				A2(
					$elm$html$Html$Events$on,
					'pointerdown',
					$elm$json$Json$Decode$succeed(
						$author$project$Main$PointerDown(word.id)))),
				$mdgriffith$elm_ui$Element$htmlAttribute(
				A2(
					$elm$html$Html$Events$on,
					'pointermove',
		...
		return A2(
			...
			A2(
				...
				A2(
					...
					_List_fromArray(
						[
							$mdgriffith$elm_ui$Element$text(word.word)
						]))));
	});
...
var $author$project$Main$view = function (model) {
	var sizes = $author$project$Main$layoutSizes(model);
	var headerFontSize = $author$project$Main$isMobile(model.viewportWidth) ? 20 : 32;
	var bodyFontSize = $author$project$Main$isMobile(model.viewportWidth) ? 16 : 20;
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$mdgriffith$elm_ui$Element$width(
						$mdgriffith$elm_ui$Element$px(sizes.gridWidth)),
				...
				_List_fromArray(
					[
						...
						$mdgriffith$elm_ui$Element$text('Connections Companion:\nSort Your Words Before Submitting!')),
						A2(
						$mdgriffith$elm_ui$Element$paragraph,
						_List_fromArray(
							[
								$mdgriffith$elm_ui$Element$Font$center,
								$mdgriffith$elm_ui$Element$Font$size(bodyFontSize)
							]),
						_List_fromArray(
							[
								$mdgriffith$elm_ui$Element$text('Enter today\'s words, drag and drop the tiles to sort them, and feel confident submitting your Connections!')
							])),
						A2(
						...
						_List_fromArray(
							[
								...
								A2($elm$html$Html$Attributes$style, '-webkit-user-select', 'none')),
								$mdgriffith$elm_ui$Element$htmlAttribute(
								$elm$html$Html$Attributes$id('word-grid')),
								$mdgriffith$elm_ui$Element$htmlAttribute(
								A2($elm$html$Html$Attributes$style, 'touch-action', 'none'))
							]),
						A2(
							...
							F2(
								function (index, word) {
									return A4($author$project$Main$viewWordTile, model, sizes, index, word);
								}),
							model.words)),
						A2(
						...
						{
							label: $mdgriffith$elm_ui$Element$Input$labelHidden('Today\'s words'),
							...
							placeholder: $elm$core$Maybe$Just(
								A2(
									...
									$mdgriffith$elm_ui$Element$text('Type today\'s words here, separated by newlines...'))),
							spellcheck: false,
							text: model.input
						}),
						model.canShowError ? $mdgriffith$elm_ui$Element$text('Please enter no more than 16 unique words, separated by newlines.') : $mdgriffith$elm_ui$Element$none
					]))
			]));
};
			...
			return A2(
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$mdgriffith$elm_ui$Element$Font$typeface('Libre Franklin'),
